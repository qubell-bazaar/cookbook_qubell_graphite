#
# Installs and configures graphite-web monitoring server
# Based on https://github.com/hw-cookbooks/graphite cookbook  
#
service "iptables" do
  action :stop
end
...
graphite_storage_schema "everything_1min_24months" do
  config ({
            priority: 100,
            pattern: ".*",
            retentions: "1m:730d"
...
graphite_web_config "#{base_dir}/webapp/graphite/local_settings.py" do
  config({
           secret_key: "#{node['qubell_graphite']['secret_key']}",
           ...
           memcache_hosts: ["#{node['memcached']['listen']}:#{node['memcached']['port']}"]
         })
